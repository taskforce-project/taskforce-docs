...
> **▶ MAJ 28/08/2026 — réconciliation registre ↔ code (audit, file:line vérifié contre `taskforce-fullstack/`).**
> **Passés ✅** (étaient encore en file ouverte) : **PC-003** (import `profile-service`), **PC-005** (webhooks
> Stripe câblés → `StripeWebhookService`), **PC-006** (`team-store` CRUD), **PC-008** (email Sales), **PC-011**
> (mock chat supprimé), **PC-013** (command-palette câblée). **PC-021bis supprimé** (doublon trompeur du PC-021
> déjà résolu, faux 🔴 P0). **PC-012 réécrit** : l'`ai-service` n'est **pas mort** — c'est une **passerelle
> FastAPI live en prod**. **Lot sécu QA-49/50/51 ajouté** (PC-036 → PC-046). **Restent OUVERTS** : PC-007
> (cache insights), PC-009 (streaming simulé), PC-014 (flags IA), PC-022/PC-023 (tx longues / écriture en
> `readOnly`), PC-028 (sous-portes CI b/c/d), PC-015 (commentaire + healthcheck).
...
| PC-003 | ✅ **Résolu** | **Import `profile-service.ts` corrigé** : `frontend/lib/api/profile-service.ts:1` = `import { apiClient } from "./client";`. | Page profil OK | S | Haute |
| PC-004 | 🟠 P1 | Refresh auth désaligné + non implémenté | Sessions non renouvelables → reconnexion forcée | M | Haute |
| PC-005 | ✅ **Résolu** | **Webhooks Stripe câblés** : `StripeWebhookController:64-70` → `StripeWebhookService` mute `Subscription`/`SubscriptionHistory` (idempotent, `StripeWebhookService.java:41-43`). | Facturation synchronisée | L | Haute |
| PC-006 | ✅ **Résolu** | **`team-store` parité CRUD** : `createTeam`/`updateTeam`/`deleteTeam` + `addMember`/`removeMember`, `frontend/lib/store/team-store.ts` (l.21-25, impl. 47-80). | UI Teams fonctionnelle | M | Moy. |
| PC-007 | 🟡 P2 | AI Insights sans cache + pas de garde quota/timeout Groq | Latence + épuisement quota | M | Haute |
| PC-008 | ✅ **Résolu** | **Inquiry Sales notifiée** : `SalesService.java:71` `emailService.sendInternalNotification(...)` (champ `:26`). | Leads remontés | S | Haute |
...
| PC-011 | ✅ **Résolu** | **Mock chat supprimé** : `frontend/components/messages/data.ts` + dossier `components/messages/` retirés (chat supprimé). | Plus de faux contenu | S | Moy. |
| PC-012 | 🟢 P3 | **Passerelle IA Python = LIVE (constat « mort » périmé)** : `ai-service/` est une passerelle FastAPI **active en prod** (routers chat/embeddings/smart_assign/health) — `docker-compose.prod.yml:158` `AI_SERVICE_URL: http://ai-service:8000`, backend `depends_on:214`. Dette réelle = **documenter** le rôle (Ollama/Groq), pas supprimer. | Doc archi à corriger | S | Moy. |
| PC-013 | ✅ **Résolu** | **Command-palette câblée** : `command-palette.tsx:152` action « Create issue » réelle, placeholder `toast.info` supprimé. | UX complétée | S | Haute |
...
| PC-022 | 🟠 **Partiel — AgentService corrigé (17/08)** | **`AgentService.run` tient une connexion DB pendant tout le LLM** (audit 16/07, vérifié) : `@Transactional` en `AgentService:78` englobe `runToolLoop:253-254`, soit jusqu'à `MAX_TOOL_ITERS=5` appels `llm.rawChat` **séquentiels**. Read-timeout du gateway = **300 000 ms** (`AiGatewayClient:40`) → **jusqu'à 25 min de connexion retenue**. Pool Hikari = **20** en prod (`application-prod.yml:12`), 10 en dev, `connection-timeout: 30000`. **20 chats Cortex simultanés = pool épuisé = toute l'API tombe** (chaque requête attend 30 s puis échoue). Même famille : `PlaneIntegrationService:103` (`sync` = N appels HTTP d'embedding non bornés dans 1 tx), `SmartAssignService:128` (`bulkRecommend` = N LLM dans 1 tx). **Le patron correct existe déjà dans le repo** : `BrainIngestionListener:43-57` (tx courte → LLM hors tx → tx courte) et `AnalysisJobRunner:44`. | API entière indisponible sous charge modérée | M | Haute |
...
| PC-036 | ✅ **Résolu (QA-51)** | **SSRF sur les webhooks sortants (OWASP A10)** : `WebhookService.fireOne` POSTait vers une URL utilisateur **sans validation**. Fix `SsrfGuard` (résolution DNS + blocage loopback/privé/CGNAT Tailscale `100.64/10`/ULA), câblé `WebhookService.java:52/:87/:120`. | Réseau interne/métadonnées cloud joignable | S | Haute |
| PC-037 | ✅ **Résolu (QA-51)** | **XSS markdown `javascript:` (A03)** : `[x](javascript:…)` exécutable au clic (chat IA + notes Brain). Fix `safeLinkUrl`/`safeImageUrl`, `lightweight-markdown.tsx:62/:68` (appliqués `:120`/`:91`). | Exécution JS au clic | S | Haute |
| PC-038 | ✅ **Résolu (QA-51)** | **Refresh token exposé au JS (A07)** : passé en cookie HttpOnly `RefreshTokenCookie` (`AuthController:54`) ; plus stocké en `localStorage`. | Vol de session par XSS | S | Haute |
| PC-039 | ✅ **Résolu (QA-49)** | **H1 — IDOR cross-tenant liens GitHub** : `GitHubIntegrationService` résolvait l'issue par ID brut. Fix `scopedIssue` (`:197`) + `assertCanWrite/assertCanView` (`:209/:231/:246`). | Fuite/écriture cross-tenant | S | Haute |
| PC-040 | ✅ **Résolu (QA-49)** | **H2 — WebSocket cross-tenant** : `projects.{id}`/`analysis.{ws}` sans contrôle. Fix `RealtimeAuthorizationService` (`StompAuthInterceptor.java:57,123` → `:146/:151`). | Fuite temps réel cross-tenant | M | Haute |
| PC-041 | ✅ **Résolu (QA-49)** | **M3 — sous-lectures projet privé** : 6 sous-lectures d'issue sans `assertCanView`. Fix `IssueService.java` `:588/:650/:853/:867/:966/:986`. | Fuite de données projet privé | S | Haute |
| PC-042 | ✅ **Résolu (QA-50)** | **M4 — token fichier Brain 32→122 bits** : clé `UUID.substring(0,8)` brute-forçable → UUID complet, `BrainAttachmentController.java:57`. | Proxy fichier devinatoire | S | Haute |
| PC-043 | ✅ **Résolu (QA-49)** | **M5 — spoofing IP du rate-limit** : confiance à la 1re valeur XFF. Fix `CF-Connecting-IP`→`X-Real-IP`→dernier hop, `RateLimitFilter.java:145`. | Contournement du rate-limit | S | Haute |
...
| PC-049 | ✅ **Résolu (25/09)** | **Vue d'audit tronquée sur téléphone** (bloc Enterprise de la home) : cadre 16:9 et 8 lignes en `flex-1` dont le contenu (~35 px) dépasse la hauteur disponible sous ~640 px, donc les dernières lignes et le pied étaient coupés par `overflow-hidden`. Fix : cadre en 4:3 et 5 lignes sous `sm` (`max-sm:hidden`), mesuré sans débordement à 1440, 1024 et en mobile, `Proof.tsx`. | Preuve d'audit illisible sur mobile | S | Haute |
| PC-050 | ✅ **Résolu (25/09)** | **Délégation à Claude Code bloquée pour toujours sans runner** : en prod, le flag du runner était allumé (override hors dépôt du 23/09), donc « Claude Code » proposé à tous. Sans runner à l'écoute, le run restait `QUEUED` indéfiniment : `DeliveryRunner.refresh` ne relisait que les runs `RUNNING`, et `ClaudeCodeProvider.poll` ne détecte qu'un runner perdu après un heartbeat. Fix : délai de réclamation (`claim-timeout-minutes`, 15) puis clôture par UPDATE conditionnel (`expireUnclaimed`, un claim concurrent gagne), issue → « Blocked » avec un message qui dit quoi faire ; le contrôleur (`latestRun`, `listRuns`) relit aussi les runs `QUEUED` (il ne relisait que les `RUNNING`) ; flag rendu durable (`docker-compose.prod.yml` + `.env.prod`). Voir ADR-013, mise à jour du 25/09. En prod v0.40.1 (PR #321, #322). | Tâche déléguée « en attente » sans fin, sans explication | S | Haute |
...
chat OK · PC-012 documenter la passerelle `ai-service/` (live en prod, plus « morte ») · PC-013 câbler le dialog create-issue · PC-014
