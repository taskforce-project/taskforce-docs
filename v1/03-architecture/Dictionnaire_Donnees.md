...
| ----------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------- |
| **Frontend**                        | Next.js + TypeScript                                | Interface utilisateur, dashboards, formulaires, expérience utilisateur |
| **UI Framework**                    | Tailwind CSS + shadcn/ui                            | Composants UI, design system, cohérence graphique                      |
| **Backend API**                     | NestJS (TypeScript) *(ou Spring Boot / .NET)*       | Logique métier, workflows, règles de validation, intégrations          |
| **API**                             | REST + OpenAPI/Swagger                              | Communication frontend/backend, documentation API                      |
| **Authentification / IAM**          | Keycloak                                            | Authentification, MFA, gestion utilisateurs, rôles, permissions (RBAC) |
| **Base de données**                 | PostgreSQL                                          | Données métier, utilisateurs, dossiers, workflows, historiques         |
| **ORM / Data Access**               | Prisma / TypeORM *(NestJS)* ou Hibernate *(Spring)* | Accès structuré aux données                                            |
| **Stockage documents**              | MinIO (compatible S3)                               | Documents clients, contrats, pièces justificatives                     |
| **Cache / traitements asynchrones** | Redis                                               | Cache, files d'attente, tâches longues                                 |
| **Backend hosting**                 | VPS / Cloud + Docker                                | Hébergement des services backend                                       |
| **Frontend hosting**                | Vercel                                              | Déploiement Next.js, CDN, CI/CD frontend                               |
| **Conteneurisation**                | Docker + Docker Compose                             | Déploiement reproductible des services                                 |
| **Reverse Proxy**                   | Nginx / Traefik                                     | Routage, HTTPS, exposition API                                         |
| **Observabilité**                   | OpenTelemetry + Grafana + Loki                      | Monitoring, logs, traces                                               |
| **CI/CD**                           | GitHub Actions                                      | Tests automatiques, déploiement continu                                |
| **Audit / Traçabilité**             | PostgreSQL Audit + Keycloak Events                  | Historique des actions, preuve de conformité                           |
