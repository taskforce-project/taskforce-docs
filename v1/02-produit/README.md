...
- **Site (25/09/2026) :** hero repositionné sur les agents de code (24/09), passe de polish (hero scindé + runs live par rôle, titres révélés au scroll, metas SEO), puis runner local présenté en accès anticipé et bloc Enterprise sur un écran (home ≈ 10 écrans). **Runner local fiabilisé en prod** : une délégation que personne ne réclame est close après 15 min, flag durable via `.env.prod`, logo Anthropic retiré du site et de l'app ([PC-050](../09-audits/Problemes_Connus.md), ADR-013). Produit **v0.40.1**. Détail : [Landing](./Landing.md) LP-HERO-001, LP-HOME-001, LP-RUNNER-001 ; [Backend](./Backend.md) BE-AGENT-DELIVERY-RUNNER.
