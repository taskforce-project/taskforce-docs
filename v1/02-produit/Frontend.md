...
- [x] **Clôture v1 — sidebar & bandeaux « Lab » (11/07/2026)** : **Agents retiré** (non construit) ; **fiole bleue** info (`FlaskConical`) sur **Intelligence** + **Brain OS** (drapeau `lab` de `NavItem`) ; **Messages déverrouillé** (chat réel) ; **Discussions reste verrouillé** (chantier séparé). Nouveau `LabBanner` (bleu façon sandbox Stripe, dismissible localStorage/feature, feedback mailto) posé sur Intelligence, Brain OS et l'onglet Intégrations. `tsc`/`eslint` verts. [id:: FE-CORE-010] [statut:: done] [parite:: extra] [ref:: frontend/components/layout/{sidebar/app-sidebar,lab-banner}.tsx] (11/07/2026)
- [x] **Durcissement pré-bêta / prêt post LinkedIn (08/09/2026)** [id:: FE-CORE-011] [statut:: done] [parite:: ok] [ref:: multiples (audit 3 axes)] — Lot de correctifs avant lancement public : **(1) bug « faux succès »** — les stores avalent l'erreur et renvoient `null/false` (pas de throw), les `catch` étaient morts → `toast.success` s'affichait même en échec (issue-sheet PERDAIT le commentaire posté) ; corrigé en branchant sur le retour (edit-project, bulk-assign, issue-sheet, settings, team-switcher ; `deleteComment` → `boolean`). **(2)** États loading/error fiabilisés (Inbox/Issues/Analytics/Membres, + champs `error`/`membersError` sur 2 stores). **(3)** Domaine périmé `taskforce.dev` → `contact@taskforce-project.fr` (contact RGPD mort) + français résiduel dans l'UI anglaise → anglais. **(4)** `global-error.tsx` (filet root-layout), liens morts `/dashboard`→`/`, racine sans workspace (état vide au lieu d'un spinner infini). **(5) Landing** : 46 CTA `/waitlist` → `${APP_URL}/auth/register` (signup ouvert), mentions légales renseignées (éditeur Pierre Michel, hébergeur Vercel/OVH+Cloudflare), footer (LinkedIn/Instagram/email + tag version), `comparison-table` dead-code supprimé. Sécu vérifiée en live (actuator ne fuit pas, anti-bot actif). Gates verts (tsc, vitest+coverage, build landing).
- [x] **Chat dé-mocké (11/07/2026)** : `components/messages/data.ts` **supprimé** ; `MessagesPage` réécrite **API-only** (plus de `localMessages`/`findChannel` — canaux/messages réels du seed, temps réel STOMP) ; `chat-sidebar`/`message-bubble`/`channel-header`/`channel-row` nettoyés (retrait `MEMBERS`, `totalUnread` inliné, avatars DM génériques). Zéro mock frontend dans le chat ; rendu réel vérifié avant refacto (canaux design/dev/general/random, auteurs Lina Park/Sarah Chen). [id:: FE-MSG-010] [statut:: done] [parite:: ok] [ref:: frontend/app/(protected)/[workspace]/messages/page.tsx] (11/07/2026)
- [x] **Fix dropdowns de l'issue-sheet (11/07/2026)** : les Select (Priority/Assignee/Points) « ne s'ouvraient plus » — `SelectContent` était en `position="item-aligned"`, cassé dans un **Radix Dialog modal** (`body pointer-events:none` + scroll-lock) : le Select passait `open` mais se positionnait hors-champ (invisible). Passé en **`position="popper"`** (défaut shadcn) → tous les Select de l'app corrigés. Vérifié en live (clic réel → menu Priorité Urgent/High/Medium/Low/None visible). [id:: FE-UI-035] [statut:: done] [parite:: ok] [ref:: frontend/components/ui/select.tsx] (11/07/2026)
- [x] **Composant `DataTable` partagé (11/07/2026)** : tableau réutilisable et paramétrable (shadcn `Table` + tri par colonne optionnel + pagination `usePagination`/`DataTablePagination` avec sélecteur rows-per-page). `DataTableColumn<T>` = { key, header, icon?, align?, className? (responsive), render, sortValue? }. **Adopté par Signals/Inbox (`inbox-view.tsx`) et My Queue (`my-work-view.tsx`)** en remplacement des listes flex custom → styles homogènes entre vues (le retour user « les tableaux ne se ressemblent pas »). Vérifié live : Inbox 35 signaux → 25/page sur 2 pages, actions Ack/Open + navigation conservées ; My Queue Issues/Sprints/Pages en colonnes triables. [id:: FE-UI-036] [statut:: done] [parite:: ok] [ref:: frontend/components/ui/data-table.tsx] (TF-UI-TABLES, 11/07/2026)
- [x] **Visite guidée (product tour) — logique de fermeture corrigée (16/08)** : fermer la visite « tôt » (croix/Échap) la **désactivait définitivement** (`close(true)` → `hasSeen`). Désormais une case **« Ne plus afficher »** (décochée à chaque (ré)ouverture) pilote le blocage : fermeture tôt = `close(dontShowAgain)` → la visite **revient au prochain accès** tant que la case n'est pas cochée ; « Terminer » (dernière étape) marque la visite comme vue. [id:: FE-TOUR-001] [statut:: done] [parite:: ok] [ref:: frontend/components/tour/product-tour.tsx + frontend/lib/store/tour-store.ts] (16/08/2026)
- [x] **`DataTable` — garde `isRowClickable` (20/07/2026)** : une ligne **sans destination** n'est ni cliquable ni pourvue du bouton « ouvrir ». Posé après le constat des **lignes mortes du Signal Center** (liens de notification `NULL` en base, cf. [PC-032](../09-audits/Problemes_Connus.md)) : la migration répare les données existantes, cette garde empêche l'UI de re-promettre un clic qui ne mène nulle part [id:: FE-UI-037] [statut:: done] [parite:: ok] [ref:: frontend/components/ui/data-table.tsx] (20/07/2026)
- [x] **« Ma file » — appels agrégés (20/07/2026)** : la vue est cross-projets et rappelait l'API **projet par projet** → `3 + 2N` requêtes par affichage (N = nombre de projets), assez pour épuiser le quota de rate limiting à l'usage normal. Branchée sur les deux nouveaux endpoints agrégés `GET …/my-cycles` et `GET …/my-pages`. **Mesuré : `3+2N` → 3 appels, 0 appel par projet** [id:: FE-MYW-011] [statut:: done] [parite:: ok] [ref:: frontend/lib/api/{cycle,page}-service.ts + components/my-work/my-work-view.tsx] [besoin-backend:: BE-MYW-010] (20/07/2026)
- [x] **Traitement du 429 (20/07/2026)** : le front n'avait **aucun** traitement du rate limiting — les stores avalaient l'erreur et l'application paraissait figée. `client.ts` affiche désormais un toast dédié avec le **délai réel**, lu dans l'en-tête `Retry-After` (exposé côté back via `CorsConfig.setExposedHeaders`, sinon masqué au JS en cross-origin) [id:: FE-CORE-014] [statut:: done] [parite:: ok] [ref:: frontend/lib/api/client.ts] (20/07/2026)
...
- [x] **Courbe Throughput dashboard → tendance journalière 30 j** : la carte Throughput appelle `getAnalyticsThroughput(slug, null, "day")` (param `bucket` ajouté au service) ; libellé « Tâches résolues / jour · 30 j ». Page Analytics inchangée (reste hebdo) [id:: FE-UI-027] [statut:: done] [parite:: ok] [ref:: frontend/app/(protected)/[workspace]/dashboard/page.tsx + lib/api/analytics-service.ts] (26/06/2026)
- [x] **Onglet Roadmap par projet** : Gantt extrait en composant partagé `RoadmapGantt` (filtrable par `projectId`) → page globale `/roadmap` + nouvel onglet `/projects/[id]/roadmap` (entre Cycles et Pages). Une seule source de vérité [id:: FE-UI-028] [statut:: done] [parite:: extra] [ref:: frontend/components/roadmap/roadmap-gantt.tsx + projects/[id]/roadmap/page.tsx + projects/[id]/layout.tsx] (26/06/2026)
- [x] **Sidebar épurée (retour état d'avant)** : retrait des entrées globales Issues/Cycles/Roadmap du groupe « Work » (ajoutées par baaee8b) — désormais accessibles via les onglets de chaque projet. Groupe « Work » = Projects · Analytics · Agents [id:: FE-UI-029] [statut:: done] [parite:: ok] [ref:: frontend/components/layout/sidebar/app-sidebar.tsx] (26/06/2026)
...
- [x] **Modal Upgrade dédié** : `UpgradeDialog` (3 plans, recommandé) monté global (`AppShell`) via `useUpgradeStore` ; CTA Pro = checkout Stripe direct ; tous les CTA recâblés (profil/switcher/members/analytics). **Enrichi des crédits IA** par plan (FREE 100k / BASIC 500k / BUSINESS 2M / ENTERPRISE illimité tokens Cortex, plafonds arrêtés 27/08 QA-46) — cohérent avec le quota par compte. [id:: FE-SUB-010] [statut:: done] [parite:: extra] [ref:: frontend/components/subscription/upgrade-dialog.tsx] (QA2-19, maj 12/07/2026)
- [x] **Settings en grand modal (façon Claude)** : la page `/settings` (nav latérale + panneaux) est refactorée en `SettingsNav` + `SettingsPanels` **exportés**, réutilisés tels quels par un **Dialog plein écran** (`components/settings/settings-modal.tsx`) piloté par `useSettingsStore` (open + section) et monté global dans `AppShell`. Ouvrable **sans navigation** : sidebar « Settings », menu utilisateur (Billing/Notifications → section deep-link), popover Cortex « Détails » → section **Usage IA**. La page `/settings` reste dispo (deep-link direct). Vérifié live (nav, changement de section, Usage IA agrégée par compte + jauge bleue, ouverture depuis chaque CTA). [id:: FE-SET-013] [statut:: done] [parite:: extra] [ref:: frontend/components/settings/settings-modal.tsx + lib/store/settings-store.ts] (12/07/2026)
- [x] **Page Facturation dédiée (façon Claude)** : route `/[workspace]/billing` (plan courant + statut + « Gérer la facturation » · **conso IA agrégée par compte** avec jauge bleue · comparatif de plans avec crédits IA), accessible depuis le **menu utilisateur** (Account/Billing/Notifications), **retirée de la sidebar principale et du modal Settings**. Code mort nettoyé (`BillingPanel`/`PLAN_FEATURES`/imports morts, `nav.billing`, lien `plan-gate`). Corrige **2× 500** : billing (`GET /api/billing/subscription`, chemin protégé, lu en `.data.data`) et status (sonde repointée `/api/workspaces/current` → `/api/workspaces`). Vérifié live (page sans toast 500, Status « tout opérationnel »). [id:: FE-BILL-020] [statut:: done] [parite:: extra] [ref:: frontend/app/(protected)/[workspace]/billing/page.tsx] (12/07/2026)
...
- [x] **Séquence refresh alignée (28/08/2026)** : refresh **single-flight** sur 401 (`refreshInFlight`), refresh token en **cookie HttpOnly** `tf_refresh` (plus en localStorage), purge de session + redirection `/auth/login` à l'échec [id:: FE-CORE-012] [statut:: done] [ref:: frontend/lib/api/client.ts] (ferme PC-004)
- [~] **Passer l'UI en anglais** (décision produit 20/07/2026 — remplace « compléter i18n FR/EN ») : l'**interface applicative** doit être en anglais ; **commentaires de code et documentation restent en français**. **Fait** : Signal Center, « Ma file » (libellés restants + messages vides), message vide par défaut de `DataTable`. **Reste (mesuré)** : **~1000 chaînes visibles dans ~110 fichiers front** [id:: FE-CORE-013] [statut:: wip] [prio:: P2] [besoin-backend:: BE-I18N-001]
- [ ] 🔴 **Unifier les deux systèmes i18n concurrents** (bloque FE-CORE-013) : `lib/i18n/index.tsx` (contexte React, `t()` **fonction**, défaut **`en`**, clé localStorage **`tf-locale`**) et `lib/store/preferences-store.ts` (Zustand, `t` = **objet**, défaut **`fr`**, clé **`taskforce-preferences`**) coexistent et se contredisent — **le sélecteur de langue des réglages ne pilote que le premier**. Environ **5 % des fichiers** utilisent l'un ou l'autre, **tout le reste code les chaînes en dur**. Traduire fichier par fichier avant d'unifier figerait la contradiction dans ~110 fichiers de plus [id:: FE-CORE-015] [statut:: todo] [prio:: P2] [ref:: frontend/lib/i18n/index.tsx · frontend/lib/store/preferences-store.ts] (20/07/2026)
...
- [x] **Validation Zod des formulaires d'auth (16/08)** : `loginSchema` + `registerSchema` (`lib/validation/auth-schemas.ts`) appliqués via `safeParse` dans `login-form.tsx` et `register-info-form.tsx` — ferme l'écart « Zod déclaré mais inutilisé » (règle d'or #8). Réutilise les helpers existants (`lib/utils/validation`) par `.refine(...)` ; retour immédiat côté client, la barrière serveur `@Valid` restant la source d'autorité. [id:: FE-AUTH-020] [statut:: done] [parite:: ok] [ref:: frontend/lib/validation/auth-schemas.ts + frontend/components/auth/{login/login-form,register/register-info-form}.tsx] (16/08/2026)
...
- [x] **Signal Center — fin des lignes mortes + passage en anglais (20/07/2026)** : **35 signaux sur 266** (ceux du seed) affichaient un identifiant (« WEB-3 ») mais **le clic ne faisait rien** — cause en base, pas dans l'UI : `notifications` n'a **aucune FK** vers `issues`/`projects`, les liens sont dénormalisés à l'écriture côté Java, donc les lignes seedées naissent avec `issue_url`/`project_url` à `NULL` (cf. [PC-032](../09-audits/Problemes_Connus.md), migration `V71`). Côté front : garde `isRowClickable` (FE-UI-037) + libellés et messages vides passés en **anglais** [id:: FE-INBOX-011] [statut:: done] [parite:: ok] [ref:: frontend/components/inbox/inbox-view.tsx] (20/07/2026)
...
- [x] **Dock « Workflows IA »** : 2ᵉ consommateur du socle `panel-store`/`PanelDock` (après « Ask AI ») — bouton topbar avec badge des analyses actives, cartes de workflow, arbre d'étapes live (`AgentPlan`), et **réponse HITL** quand le modèle demande une précision [id:: FE-ANL-001] [statut:: done] [parite:: extra] [ref:: frontend/components/workflows/{workflow-dock,workflows-button}.tsx · lib/store/workflow-store.ts] (10/07/2026)
- [x] **DecisionBoard actionnable** : un bouton « Analyser » + bascule Rapide/Approfondi lance un **workflow asynchrone** (on peut quitter la page) ; le brief persisté est rechargé à l'aboutissement, et chaque priorité porte ses actions — **Créer l'issue** (lien vers l'issue créée), épingler, éditer, écarter/restaurer [id:: FE-ANL-002] [statut:: done] [parite:: extra] [ref:: frontend/components/analytics/decision-board.tsx] (10/07/2026)
- [x] **Deep-link d'issue sur le board** : `/{slug}/projects/{id}?issue={issueId}` ouvre le kanban **avec la sheet de l'issue**, au lieu de la page détail isolée. Tous les liens entrants y pointent désormais (décision IA, My Work / notifications, « Copier le lien » de la sheet) ; « Ouvrir en plein écran » mène toujours à la page détail. Le lien est consommé **une seule fois** puis le paramètre retiré de l'URL — sinon le temps réel rejoue l'effet et rouvre l'issue du lien par-dessus celle qu'on venait d'ouvrir [id:: FE-ISSUE-040] [statut:: done] [parite:: ok] [ref:: app/(protected)/[workspace]/projects/[id]/page.tsx] (10/07/2026)
- [x] **Page Intelligence (ex-« Operational Intelligence ») réorganisée** : `PageHeader`, tout en français, sélecteur de projet rendu visible (barre de contexte dédiée). L'**IA d'abord** (décision du jour, puis signaux), les chiffres ensuite. Tailles de police remontées (fin des `text-[10px]` empilés). Cartes KPI **alignées sur le style dashboard** : `SectionCard` (en-tête gris + icône) + `MetricSplit` au lieu de cartes plates [id:: FE-ANA-030] [statut:: done] [parite:: ok] [ref:: app/(protected)/[workspace]/analytics/page.tsx] (10/07/2026)
- [x] **Explorateur de graphes AI-driven** : une **carte unique en 3 volets** (débit · burndown · charge), style dashboard avec dégradés (`AreaChart` + `linearGradient`). Un clic ouvre un **bottom sheet** plein-largeur (vaul `Drawer direction="bottom"`, façon console développeur Stripe) : sidebar gauche = catalogue de graphes sur données réelles + **input IA** (« génère avec l'IA »), zone principale = le graphe rendu en grand (hauteur pleine). Un renderer unique `SpecChart` rend n'importe quelle `ChartSpec` depuis les vraies séries (`area|bar|line` + heatmap pour la charge) ; l'IA ne fournit que le cadrage, jamais les données. Gaté Pro (les datasets sont `ADVANCED_ANALYTICS`) : volets verrouillés + CTA upgrade pour FREE [id:: FE-CHART-001] [statut:: done] [parite:: extra] [ref:: frontend/components/analytics/chart-explorer.tsx] (10/07/2026)
  - **Mode répartition (retrieval DB) + graphes épinglés (10/07/2026)** : `SpecChart` rend le mode `breakdown` (barres/ligne depuis les données calculées en base) en plus des séries temporelles. La sidebar du sheet a **deux sections distinctes** : **« Permanents »** (les graphes de base du catalogue, toujours là) et **« Épinglés »** (les graphes générés par l'IA que l'utilisateur sauvegarde — section **toujours visible** avec un état vide explicite « génère un graphe puis épingle-le »). Le bouton **Épingler** n'apparaît **que sur un graphe généré par l'IA** (`canPin = aiGenerated && mode !== "unsupported"`) — épingler un permanent n'a pas de sens ; clic pour recharger, corbeille pour retirer. Un graphe épinglé breakdown se **rafraîchit** via `runBreakdown` (données à jour, pas figées).
  - **Graphs interactifs + 3D (10/07/2026)** : tooltip riche partagé (`ChartTooltip` — catégorie + pastille couleur par série + valeur formatée), **grille** dans les deux sens, **légende cliquable** (masquer/afficher une série), **surlignage au survol** (`activeBar`/`activeDot` + curseur). Heatmap : légende d'intensité **remontée en tête** (+ « pic ») et cellules surlignées au survol. **Vue 3D** (toggle 2D/3D) pour les répartitions en barres — `Bars3D` (Three.js, dép. déjà présente ; `@types/three` ajouté) : barres 3D sur sol quadrillé, rotation (drag + auto), survol → surlignage + valeur ; composant défensif (dispose/RAF/ResizeObserver au démontage) [ref:: frontend/components/analytics/bars-3d.tsx]. ⚠️ *Rendu 3D non vérifié visuellement (login navigateur requis) — à contrôler.*
  - **Palette colorée** : les séries recharts utilisent une palette sémantique explicite (résolues=emerald, ouvertes=blue, restant=rose, idéal=slate, charge=indigo) — les tokens `--chart-*` du thème sont volontairement en niveaux de gris, d'où des courbes monochromes sans override.
  - **Heatmap charge : état vide clair** — `WorkloadHeatmap` affiche un message explicite quand aucune échéance n'est planifiée sur la fenêtre (toutes cellules à 0), au lieu d'une grille grise qui semble cassée. Panneau du sheet scrollable (jusqu'à ~33 membres). *(Constat data : le seed n'a pas d'échéances à venir → le heatmap est vide ; « Charge par membre » (capacity, open issues) reste rempli.)*
  - **Correctif « pas de grille/légende/tooltip » — cause racine (10/07/2026, vérifié en live via l'extension Brave)** : les axes/grille/légende/tooltip étaient regroupés dans un **`<Fragment>`** (`const axes = <>…</>`) injecté dans chaque graphe recharts. **Recharts n'introspecte que ses enfants directs et aplatit les tableaux, mais n'entre pas dans un Fragment** → Grid/XAxis/YAxis/Tooltip/Legend étaient silencieusement ignorés ; seules les séries (tableau `.map()`) s'affichaient, d'où le graphe « nu ». Correctif : `axes` devient un **tableau** d'éléments (avec `key`), pas un `<>…</>` — dans `SpecChart` (aire/barre/ligne) **et** `BreakdownChart`. Preuve DOM (Débit hebdo) : grille 13 lignes, axe X 8 ticks (S1–S8), axe Y 5 ticks, légende « Résolues/Ouvertes », tooltip « S4 · Résolues 9 · Ouvertes 27 » au survol. *(Les marges/`overflow` d'une itération précédente n'étaient pas la cause.)*
  - **Tooltip heatmap façon GitHub (10/07/2026)** : `WorkloadHeatmap` remplace le `title=` natif par une **carte de survol** (nb d'échéances au pluriel + membre + date lisible), 8px au-dessus de la cellule, centrée. Piège CSS corrigé : `position: fixed` est cassé sous la Drawer vaul (ancêtre `transform` → le bloc conteneur de `fixed` n'est plus le viewport, décalage ~+73px) → tooltip en **`absolute`** dans un wrapper `relative`, coordonnées via différences de `getBoundingClientRect` (insensible au transform). Vérifié en live : « 2 échéances · Diego Santos · mardi 21 juillet », posé au-dessus, centré.

**À faire :**
- [ ] **Abonner le dock à STOMP** `/topic/analysis.{workspaceId}` (le backend publie déjà). Bloqueur : `lib/hooks/use-stomp.ts` est couplé au chat (ne s'abonne qu'à `/topic/channel.{id}`) — extraire la connexion + fallback SockJS avant d'ajouter un abonnement, plutôt que dupliquer ~90 lignes. En attendant : polling 5 s tant qu'un workflow est actif [id:: FE-ANL-003] [statut:: todo] [parite:: extra] [prio:: P2] [besoin-backend:: néant] (10/07/2026)
...
**Fait :**
- [x] **Catalogue d'intégrations entièrement connectable** : le marketplace (`IntegrationsCatalog`) rend les 47 connecteurs `AVAILABLE` (fin des cartes « Bientôt » mortes). GitHub/Slack gardent l'OAuth 1-clic, Plane sa sync Brain OS ; **tous les autres passent par un dialog générique** (`ConnectorDialog`) : formulaire déduit du mode d'auth → `connectConnector` (identifiants stockés chiffrés), état connecté + déconnexion. [id:: FE-INT-010] [statut:: done] [parite:: extra] [ref:: frontend/components/integrations/integrations-catalog.tsx · lib/api/integration-service.ts] (10/07/2026)

- [x] **Aperçu des pièces jointes réparé — la CSP bloquait, pas MinIO (20/07/2026)** : vignette cassée dans le sheet d'issue, `fetch` de l'URL présignée en `TypeError: Failed to fetch`. **Tout le soupçonné était sain** (`/attachments` 200, MinIO joignable, signature présignée valide — URL générée avec `mc` → 200 / 1449 octets, CORS MinIO correct). **Vraie cause** : `next.config.ts` construisait la CSP **sans l'origine du stockage objet** — `https:` dans `img-src` **ne couvre pas** un MinIO local en `http://localhost:9000`, et `connect-src` ne le listait pas. **Correctif** : constante `STORAGE_ORIGIN` (`NEXT_PUBLIC_STORAGE_URL`, défaut `http://localhost:9000`) ajoutée à **`img-src` ET `connect-src`** + variable passée au service `frontend` dans `docker-compose.dev.yml`. ⚠️ **doit rester alignée sur `MINIO_PUBLIC_ENDPOINT`** côté backend (c'est l'hôte signé). Vérifié en live (vignette du logo, issue 4826) [id:: FE-ATT-010] [statut:: done] [parite:: ok] [ref:: frontend/next.config.ts · docker-compose.dev.yml] (PC-031, 20/07/2026)
...
- [x] **Suite front verte + couverture re-mesurée (21/07/2026)** — `vitest run` : **785 tests / 62 fichiers, 0 échec** ; `--coverage` : **88,83 % lignes, 87,24 % branches, 87,04 % fonctions**, sortie 0 (seuil grille : 50 %). Les 41 tests qui échouaient décrivaient tous un **contrat périmé**, jamais un bug : option `silentError` ajoutée depuis, routes Stripe passées sous `/api/billing`, catalogue de plans passé de 3 à 4 (tarification par siège), logout devenu un rechargement dur (QA2), `updateIssue` ne posant plus `store.error` (WS-10). Corrigés au passage dans `plan-form.tsx` : union de types désalignée du catalogue (masquée par un `as`) et grille à 3 colonnes pour 4 cartes. **50 tests ajoutés** sur les 4 stores qui n'en avaient aucun (`workflow`, `dashboard-cards`, `brain`, `settings`) et **`client.test.ts` réécrit** — il mockait axios intégralement, donc les intercepteurs n'étaient jamais exécutés ; ils sont désormais récupérés depuis `use.mock.calls` et testés (Bearer, refresh 401 + rotation, purge de session, toasts réseau/5xx/429). [id:: FE-QA-001] [statut:: done] [parite:: ok] [ref:: frontend/vitest.config.ts] (C3, `.ai/roadmap.md` §4.A)
...
**Dernière mise à jour :** 16/08/2026 · **Projet :** Taskforce — Metz Numeric School 2025-2026
