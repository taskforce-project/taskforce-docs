...
- [x] **Passe de polish du site inspirée de relevanceai.com (25/09/2026, landing-v1.2.0, v0.39.0)** [id:: LP-HOME-001] [statut:: done] [parite:: ok] [prio:: P1] : audit mesuré d'abord (92 pages, 5 largeurs) puis 4 emprunts validés par le CEO. **Hero scindé** avec une île `HeroRuns.tsx` (`client:idle`) : table de runs animée, onglets par rôle (Engineering, Product, Operations, Client services), marquée « Illustrative », figée si reduced-motion. **Home 14,3 → 11,0 écrans** : Problem fusionné dans Before/After, Smart Assign dans « What ships today », mémoire dans « Why it's different », « The direction » sous « Who it's for », doublons retirés, 7 checkpoints de The run repliables (`<details>`). **Titres de section révélés au scroll** (`src/scripts/reveal-titles.ts`, plancher d'opacité 0,55 pour rester AA). **Mobile** : frise de The run verticale sous 640 px ([PC-048](../09-audits/Problemes_Connus.md)). 10 composants morts supprimés. Vérifié : lint (6 warnings préexistants), build 94 pages, sitemap 91 URLs en 200, marqueurs présents en prod. PR #317 → `dev`, PR #318 → `main`. **MAJ 25/09/2026 (2e passe, v0.40.0)** : Enterprise sur un écran (texte + garanties | vue d'audit ; 1,87 → 0,90 écran à 1440×900, home 12,3 → 11,3 à cette taille, soit ≈ 10 sur la base de mesure initiale) ; passe ponctuation terminée (38 paragraphes `t-lead` + 2 `support=` oubliés par la 1re passe) ; vue d'audit lisible sur téléphone ([PC-049](../09-audits/Problemes_Connus.md)). PR #319 → `dev`, PR #320 → `main`, en prod `landing-v1.3.0`. [ref:: landing-page/src/components/site/home/ + landing-page/src/pages/index.astro + .ai/design-grammar.md §7]
- [x] **Runner local présenté sur le site, en accès anticipé (25/09/2026, v0.40.0)** [id:: LP-RUNNER-001] [statut:: done] [parite:: ok] [prio:: P1] : le runner (ADR-013) tourne en prod derrière un flag, mais le site ne disait pas comment la délégation se fait. Section `#local-runner` sur `/product/orchestration` (badge Beta) : les 4 étapes réelles (Assign, Claim, Build, Review), ce que l'agent peut toucher (dépôt, poste, TaskForce, GitHub, repris du README du runner), le mode sans dépôt, la consigne glissée bornée par le périmètre, les prérequis et « TaskForce never sees your Claude credentials » ; bouton « Ask for early access ». Liens depuis la carte Handoff et Smart Assign, item Beta dans « Next » de la roadmap, `llms.txt`, FAQ de la home. Conditions d'Anthropic : Claude Code nommé en texte seulement, jamais comme nom de fonctionnalité, sans logo. Accès anticipé = runner provisionné par l'équipe, pas de libre-service tant que « Connect a runner » n'est pas livré. Aussi listé dans le groupe Beta de `/changelog`. PR #319 → `dev`, PR #320 → `main`, vu en prod le 25/09 (`landing-v1.3.0`). [ref:: landing-page/src/pages/product/orchestration.astro + taskforce-runner/README.md]
