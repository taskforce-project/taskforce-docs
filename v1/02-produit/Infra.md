...
- [x] **Dashboard PRODUIT Grafana (08/09/2026)** [id:: INF-OBS-002] [statut:: done] [parite:: ok] [ref:: monitoring/vm1/metrics/grafana-metrics.sql + monitoring/vm2/grafana] — Suivi de l'évolution produit (utilisateurs, répartition par plan, inscriptions dans le temps, dernière activité externe) via des **vues read-only** `metrics.*` + rôle `grafana_ro` (SELECT borné au schéma `metrics` ; vues *definer* → **aucun accès aux tables `public.*`**, vérifié : `permission denied for table users`) + relais **socat 9102** lié à l'IP Tailscale. Datasource Postgres + dashboard « TaskForce - Produit » provisionnés (health « Database Connection OK »). **Exclut** le compte de Pierre + la démo (`keycloak_id 'seed-%'`, workspaces `demo`/`nimbus`). Déployé à la main en SSH (hors arbre git, comme la stack monitoring `INF-OBS-001`).
- [x] **Seed démo prod « Demo Workspace » versionné (08/09/2026)** [id:: INF-SEED-001] [statut:: done] [parite:: ok] [ref:: backend/tf-api/seed/prod_demo_seed.sql] — Remplace l'ancien seed « Nimbus » ad-hoc par un fichier **versionné + idempotent** (une seule transaction `DO` atomique). Workspace slug `demo` (id=17) owné par le **compte réel de Pierre**, **sans mutation Stripe/plan** (section billing du seed dev retirée : la page Billing montre son plan réel). Coéquipiers `data-only` (`keycloak_id 'seed-%'`) → exclus des métriques `INF-OBS-002`.
