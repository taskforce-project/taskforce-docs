...
> **État d'implémentation (10/09/2026, branche `v2`).** Le squelette est **livré et prouvé** : lien projet↔dépôt ([[../02-produit/Backend|BE-AGENT-DELIVERY-1]], en prod), registre de providers (`-2`), pipeline `delegate→dispatch→poll→résultat` prouvé par un **stub** (`-3`, chemin démo zéro coût, en prod #262), auto-move kanban « In review by AI » / « Blocked » (`-4`), UI de délégation sur l'issue (`-5`). **Trois providers réels exécutables** : (1) **`ClaudeApiProvider`** (`claude-api`, B1) - clé Anthropic collée (chiffrée, `IntegrationProvider.ANTHROPIC`), **appel Messages API synchrone** facturé sur le compte user, sortie texte = résumé livrable ; (2) **`CopilotProvider`** (`github-copilot`) - **inférence hébergée GitHub** (OpenAI-compatible) via le token GitHub déjà connecté, synchrone ; (3) **`CursorProvider`** (`cursor`) - **Background Agents API**, **asynchrone** (`dispatch`/`poll`→PR) sur le dépôt lié, clé Cursor chiffrée (`IntegrationProvider.CURSOR`). Le pipeline gère donc **sync ET async** (re-poll via `DeliveryRunner.refresh` sur lecture du run). **Prouvé par `MockRestServiceServer`** ; ⚠️ **non vérifié en live** pour Cursor/Copilot (pas de compte de test) → opt-in, échec gracieux. **Manque encore** (= `claude-code` « à venir ») : Claude Code agentique + coding-agent Copilot qui **ouvrent une PR** (clone dépôt + édits + PR via Managed Agents / assignation d'issue, §7), reco de modèle au dispatch, picker façon command-menu, affichage de la conso.
