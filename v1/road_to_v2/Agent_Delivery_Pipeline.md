...
> **État d'implémentation (10/09/2026, branche `v2`).** Le squelette est **livré et prouvé** : lien projet↔dépôt ([[../02-produit/Backend|BE-AGENT-DELIVERY-1]], en prod), registre de providers (`-2`), pipeline `delegate→dispatch→poll→résultat` prouvé par un **stub** (`-3`, chemin démo zéro coût, en prod #262), auto-move kanban « In review by AI » / « Blocked » (`-4`), UI de délégation sur l'issue (`-5`). **B1 = premier provider réel exécutable** : `ClaudeApiProvider` (`claude-api`) - l'utilisateur **colle sa clé API Anthropic** (stockée chiffrée, `IntegrationProvider.ANTHROPIC`, sans migration) et TaskForce fait **un appel Messages API** synchrone facturé sur son compte ; la sortie texte = résumé livrable (task-agnostic). Prouvé par `MockRestServiceServer` (le CEO n'a pas de crédits API pour tester en live). **Manque encore** (= `claude-code` « à venir ») : clone de dépôt + édits agentiques + ouverture de PR via **Managed Agents** (§7), reco de modèle au dispatch, picker façon command-menu, affichage de la conso.
